...
import gradio as gr
import message_constructor as mc
...
class ChatHistory:
    ...
    def __init__(self):
        ...
        self.add_message(
            mc.construct_text_message(text=SYSTEM_PROMPT, role="assistant"))
...
def _yield_text_with_audio(text):
    yield text
    audio_path = tts.tts(text)
    yield [gr.Audio(audio_path), text]
...
def get_response(message, history):
    ...
    if not message:
        return "The message is empty. How about a picture?"
    ...
    if message["files"]:
        ...
        chat_history.add_message(
            mc.construct_text_message(picture_description, role="assistant"))

        yield from _yield_text_with_audio(picture_description)
        return
    ...
    chat_history.add_message(mc.construct_text_message(message["text"]))
    chat_history.add_message(reply_message)

    yield from _yield_text_with_audio(reply_text)
